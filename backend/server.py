...
async def generate_simulation(request: PromptRequest, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(run_simulation_workflow, request.prompt, timestamp_id)
...
async def get_gif(timestamp_id: str):
    ...
    if not os.path.exists(gif_path):
        ...
        if os.path.exists(os.path.join(output_dir, "error.log")):
             raise HTTPException(status_code=400, detail="Simulation failed. Check status.")
        raise HTTPException(status_code=404, detail="GIF not ready or ID not found")
